...
def run_training_node1(dataset_name):
    ...
    evaluation_log = f"evaluation_metrics_{dataset_name}.txt"
    training_log = f"training_metrics_{dataset_name}.txt"
    flag_file = "/tmp/node_1_completed.flag"
    dataset_type = "IOTBOTNET" if "IOTBOTNET" in dataset_name else "CICIOT"

    command = f"python3 clientPoisoned.py --dataset {dataset_type} --node 1 --pData {dataset_path} --evaluation_log {evaluation_log} --training_log {training_log}"
...
def run_training_node2(dataset_name):
    ...
    evaluation_log = f"evaluation_metrics_{dataset_name}_CLEAN.txt"
    training_log = f"training_metrics_{dataset_name}_CLEAN.txt"
    flag_file = "/tmp/node_2_completed.flag"
    dataset_type = "IOTBOTNET" if "IOTBOTNET" in dataset_name else "CICIOT"

    command = f"python3 clientPoisoned.py --dataset {dataset_type} --node 2 --pData {dataset_path} --evalLog {evaluation_log} --trainLog {training_log}"
