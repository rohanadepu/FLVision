...
full_feature_scores = cross_val_score(final_model, X_train_data, y_train_data, cv=cv, scoring='accuracy')
print(f"Full feature set accuracy: {full_feature_scores.mean()}")

# Evaluate using the reduced feature set rfe
reduced_feature_scores_rfe = cross_val_score(final_model, X_reduced[top_features_rfe], y_train_data, cv=cv, scoring='accuracy')
print(f"Reduced feature set rfe accuracy: {reduced_feature_scores_rfe.mean()}")

# Evaluate using the reduced feature set rf
reduced_feature_scores_rf = cross_val_score(final_model, X_reduced[top_features_rf], y_train_data, cv=cv, scoring='accuracy')
print(f"Reduced feature set rf accuracy: {reduced_feature_scores_rfe.mean()}")

# Evaluate using the reduced feature set mi
reduced_feature_scores_mi = cross_val_score(final_model, X_reduced[top_features_mi], y_train_data, cv=cv, scoring='accuracy')
print(f"Reduced feature set mi accuracy: {reduced_feature_scores_rfe.mean()}")

# Evaluate using the reduced feature set combined
reduced_feature_scores_combine = cross_val_score(final_model, X_selected, y_train_data, cv=cv, scoring='accuracy')
print(f"Reduced feature set combined accuracy: {reduced_feature_scores_rfe.mean()}")
...
# Full feature set
precision_full, recall_full, f1_full = evaluate_model(final_model, X_train_data, y_train_data, cv)
print(f"Full feature set - Precision: {precision_full}, Recall: {recall_full}, F1 Score: {f1_full}")

# Reduced feature set rfe
precision_reduced, recall_reduced, f1_reduced = evaluate_model(final_model, X_reduced[top_features_rfe], y_train_data, cv)
print(f"Reduced feature set rfe - Precision: {precision_reduced}, Recall: {recall_reduced}, F1 Score: {f1_reduced}")

# Reduced feature set rf
precision_reduced, recall_reduced, f1_reduced = evaluate_model(final_model, X_reduced[top_features_rf], y_train_data, cv)
print(f"Reduced feature set rf - Precision: {precision_reduced}, Recall: {recall_reduced}, F1 Score: {f1_reduced}")

# Reduced feature set mi
precision_reduced, recall_reduced, f1_reduced = evaluate_model(final_model, X_reduced[top_features_mi], y_train_data, cv)
print(f"Reduced feature set mi - Precision: {precision_reduced}, Recall: {recall_reduced}, F1 Score: {f1_reduced}")

# Reduced feature set combined
precision, recall, f1 = evaluate_model(final_model, X_selected, y_train_data, cv)
print(f"Selected features - Precision: {precision}, Recall: {recall}, F1 Score: {f1}")
