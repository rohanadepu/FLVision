...
# Setup Flight topology
topo = Topology()
leader = topo.add_node(kind="leader")
worker = topo.add_node(kind="worker")
topo.add_edge(leader.idx, worker.idx)
print(f"Created topology with Leader idx={leader.idx} and Worker idx={worker.idx}")

# Assign dataset to worker
dataset = LazyDataset(train_files, FEATURE_COLS)
dataloader = DataLoader(dataset, batch_size=64, shuffle=True, num_workers=0)

# Create model and optimizer for worker
worker_model = NIDSModule()
optimizer = worker_model.configure_optimizers()

# Manual federated-like training
print("\n>>> Starting federated-like serial training...")
worker_model.train()
...
print("\n Manual federated-like training complete!")
